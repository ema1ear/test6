...
function renderInline(source, intent, lang) {
    ...
    html = html
        ...
        .replace(/\[\[ACTION:(.+?)\]\]/g, (_match, label) => (
            `<a href="${lang === "ca" ? "/ca/contacte/" : "/es/contacto/"}?intent=${intent}" class="btn-inline-option" data-action="contact-from-narrative" data-intent="${intent}">${label}</a>`
        ));
    return html;
}

function renderRoute(source, intent, lang) {
    const paragraphs = source.trim().split(/\r?\n\s*\r?\n/);
    return paragraphs
        .map((paragraph, index) => `<p class="scrolling-p" data-narrative-unit="${intent.toLowerCase()}-${String(index + 1).padStart(2, "0")}">${renderInline(paragraph, intent, lang)}</p>`)
        .join("");
}
...
Un responsable d’operacions va demanar formació en *prompt engineering*. Perdia temps buscant informació i preparant documents. Li vaig construir un sistema privat, replicable per altres professionals.
...
export const routeContent = {
    ca: {
        A: renderRoute(fastCa, "A", "ca"),
        B: renderRoute(narrativeCa, "B", "ca")
    },
    es: {
        A: renderRoute(fastEs, "A", "es"),
        B: renderRoute(narrativeEs, "B", "es")
    }
};
