...
function renderSources(sources, label) {
    ...
    return `<aside class="scrolling-p narrative-sources" aria-label="${escapeHtml(label)}"><span>${escapeHtml(label)}</span><ul>${sources.map((source) => `<li><a href="${escapeHtml(source.url)}" target="_blank" rel="noopener noreferrer">${escapeHtml(source.label)}</a></li>`).join("")}</ul></aside>`;
}

function renderRoute(source, intent, sources, sourcesLabel) {
    const paragraphs = source.trim().split(/\r?\n\s*\r?\n/);
    return paragraphs
        .map((paragraph) => `<p class="scrolling-p">${renderInline(paragraph, intent)}</p>`)
        .join("") + renderSources(sources, sourcesLabel);
}
