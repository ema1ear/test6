...
try {
    const cloudflareWithoutLegal = spawnSync(process.execPath, ["scripts/build.mjs"], {
        cwd: projectRoot,
        encoding: "utf8",
        env: {
            ...baseEnv,
    ...
    assert.notEqual(cloudflareWithoutLegal.status, 0, "Producción no debe desplegarse sin el contenido legal.");
    assert.match(cloudflareWithoutLegal.stderr, /Cloudflare Pages requiere LEGAL_PUBLISH=true/);
    ...
    assert.ok(
        caContact.indexOf('data-form-step="identity"') < caContact.indexOf('id="form-privacy"')
        && caContact.indexOf('id="form-privacy"') < caContact.indexOf('id="website"'),
        "La primera capa de privacidad debe aparecer en el último paso y antes del envío."
    );
